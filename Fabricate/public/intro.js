...
function join_game() {
    ...
    roomCode = document.getElementById('game-code').value;
    ...
    if (nickname) {
        if (roomCode) {
            showNotification(`Joining game with code ${roomCode}...`);
            // Placeholder for joining game logic
            setTimeout(() => window.location.replace("/lobby?roomCode=" + roomCode + "&isHost=false"), 2000);
        } else {
