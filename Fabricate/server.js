...
const app = express();
const ABLY_API_KEY = 'Dih4RQ.K9cZjQ:6X6UHFge4DcUUVIl21kv6mimDerBS7GzE7S_PUDJb4U'
const PORT = 8080;
...
app.get("/lobby", (request, response) => {
  ...
  if (!isReqHost && activeGameRooms[requestedRoom]) {
    if (
      ...
      response.sendFile(__dirname + "/pages/lobby.html");
    } 
  } else if (isReqHost) {
    response.sendFile(__dirname + "/pages/lobby.html");
  } 
...
app.get("/waiting", (request, response) => {
  response.sendFile(__dirname + "/pages/waiting.html");
});

app.get("/flag-articles", (request, response) => {
  response.sendFile(__dirname + "/pages/flag-articles.html");
});

app.get("/choose-article", (request, response) => {
  response.sendFile(__dirname + "/pages/choose-article.html");
});
